...
public class VideoMusicAdapter extends Adapter<ViewHolder> {
    public Context mContext;
    public int mCurrentIndex = 0;
    public List<ListItem> mList;

    public VideoMusicAdapter(List<ListItem> list, Context context) {
        this.mList = list;
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int i) {
        super.onBindViewHolder((VideoMusicAdapter) viewHolder, i);
        ...
        if (type == itemType || (i == this.mCurrentIndex && i == 0)) {
        ...
        if (TextUtils.isEmpty(listItem.getTitle())) {
            if (i == 0) {
                viewHolder.magicImage.setContentDescription(viewHolder.itemView.getResources().getString(R$string.acc_magic_shanchupeiyue));
            } else if (i == 1) {
                viewHolder.magicImage.setContentDescription(viewHolder.itemView.getResources().getString(R$string.acc_magic_xuanzepeiyue));
            }
        }
        if (i == 1) {
    ...
    public void selectMusicItem(int i) {
        this.mList.get(this.mCurrentIndex).setType(ListItem.ItemType.NONE);
        this.mList.get(i).setType(ListItem.ItemType.EDIT);
        notifyItemChanged(this.mCurrentIndex);
        notifyItemChanged(i);
        this.mCurrentIndex = i;
    ...
    public void cancel() {
        ListItem.ItemType type = this.mList.get(1).getType();
        if (type == ListItem.ItemType.NONE) {
            this.mList.get(this.mCurrentIndex).setType(ListItem.ItemType.EDIT);
        } else {
            ListItem.ItemType itemType = ListItem.ItemType.EDIT;
            if (type == itemType) {
                this.mList.get(1).setType(itemType);

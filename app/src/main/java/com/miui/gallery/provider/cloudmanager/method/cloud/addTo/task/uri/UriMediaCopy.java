...
public class UriMediaCopy extends BaseCopyAndMoveByMediaUri {
    public UriMediaCopy(Context context, ArrayList<Long> arrayList, String str, Cursor cursor, long j) {
    ...
    public long execute(SupportSQLiteDatabase supportSQLiteDatabase, MediaManager mediaManager) throws Exception {
        if (this.mCloudId > 0) {
            return CopyAndMoveByIdFactory.create(0, this.mContext, getDirtyBulk(), this.mCloudId, this.mAlbumId, supportSQLiteDatabase).run(supportSQLiteDatabase, mediaManager);
        }
    ...
    public String toString() {
        return String.format(Locale.US, "CopyByUri{%s, %d}", this.mTarPath, Long.valueOf(this.mAlbumId));
    }
}

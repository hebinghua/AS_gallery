...
public class TextElementOperationDrawable extends Drawable {
    ...
    public int mMinSize = Integer.MAX_VALUE;
    ...
    public TextElementOperationDrawable(Resources resources) {
    ...
    public void configDecorationPositon(RectF rectF, Matrix matrix, float f, float f2, float f3) {
        float width = rectF.height() != 0.0f ? rectF.width() / rectF.height() : 1.0f;
        int i = this.mMinSize;
        float f4 = i / width;
        float f5 = i * width;
        float f6 = i >> 1;
        if (rectF.width() < this.mMinSize) {
            rectF.left = f2 - f6;
            rectF.right = f2 + f6;
            float f7 = f4 / 2.0f;
            rectF.top = f3 - f7;
    ...
    public void configActionPosition(Action action, Action action2, Action action3, Action action4, Resources resources) {
        ...
        for (Drawable drawable : this.mDecorationDrawables) {
            if (drawable != null) {
                this.mMinSize = drawable.getIntrinsicWidth();
                return;
            }
        }
    }

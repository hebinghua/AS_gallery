...
public class OpenGlUtils {
    public static int loadTexture(Bitmap bitmap, int i, boolean z) {
        ...
        if (bitmap == null || bitmap.isRecycled()) {
            ...
            DefaultLogger.d("OpenGlUtils", "img is null: %b", Boolean.valueOf(z2));
            return -1;
        }
        int[] iArr = new int[1];
        if (i == -1) {
        ...
        if (bitmap != null) {
            ...
            try {
                if (bitmap.getWidth() > bitmap.getHeight()) {
                    return Bitmap.createScaledBitmap(bitmap, maxTextureSize, (bitmap.getHeight() * maxTextureSize) / bitmap.getWidth(), true);
                }
                return Bitmap.createScaledBitmap(bitmap, (bitmap.getWidth() * maxTextureSize) / bitmap.getHeight(), maxTextureSize, true);
            } catch (OutOfMemoryError e) {
                DefaultLogger.e("OpenGlUtils", "ensureBitmapSize error:" + e.toString());

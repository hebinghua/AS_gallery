...
public class GifDrawableTransformation implements Transformation<GifDrawable> {
    ...
    public Resource<GifDrawable> transform(Context context, Resource<GifDrawable> resource, int i, int i2) {
        GifDrawable mo237get = resource.mo237get();
        Resource<Bitmap> bitmapResource = new BitmapResource(mo237get.getFirstFrame(), Glide.get(context).getBitmapPool());
        Resource<Bitmap> transform = this.wrapped.transform(context, bitmapResource, i, i2);
        if (!bitmapResource.equals(transform)) {
            bitmapResource.recycle();
        }
        mo237get.setFrameTransformation(this.wrapped, transform.mo237get());
        return resource;
    }

...
public class DrawableTransformation implements Transformation<Drawable> {
    ...
    public Resource<Drawable> transform(Context context, Resource<Drawable> resource, int i, int i2) {
        ...
        Resource<Bitmap> convert = DrawableToBitmapConverter.convert(bitmapPool, mo237get, i, i2);
        if (convert == null) {
            if (!this.isRequired) {
                return resource;
            }
            throw new IllegalArgumentException("Unable to convert " + mo237get + " to a Bitmap");
        }

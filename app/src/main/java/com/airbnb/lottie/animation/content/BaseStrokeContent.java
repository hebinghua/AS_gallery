...
public abstract class BaseStrokeContent implements BaseKeyframeAnimation.AnimationListener, KeyPathElementContent, DrawingContent {
    ...
    public BaseStrokeContent(LottieDrawable lottieDrawable, BaseLayer baseLayer, Paint.Cap cap, Paint.Join join, float f, AnimatableIntegerValue animatableIntegerValue, AnimatableFloatValue animatableFloatValue, List<AnimatableFloatValue> list, AnimatableFloatValue animatableFloatValue2) {
        ...
        this.opacityAnimation = animatableIntegerValue.mo193createAnimation();
        this.widthAnimation = animatableFloatValue.mo193createAnimation();
        if (animatableFloatValue2 == null) {
            this.dashPatternOffsetAnimation = null;
        } else {
            this.dashPatternOffsetAnimation = animatableFloatValue2.mo193createAnimation();
        }
        this.dashPatternAnimations = new ArrayList(list.size());
        this.dashPatternValues = new float[list.size()];
        for (int i = 0; i < list.size(); i++) {
            this.dashPatternAnimations.add(list.get(i).mo193createAnimation());
        }
    ...
    public final void applyTrimPath(Canvas canvas, PathGroup pathGroup, Matrix matrix) {
        ...
        float floatValue = (pathGroup.trimPath.getOffset().mo190getValue().floatValue() * length) / 360.0f;
        float floatValue2 = ((pathGroup.trimPath.getStart().mo190getValue().floatValue() * length) / 100.0f) + floatValue;
        float floatValue3 = ((pathGroup.trimPath.getEnd().mo190getValue().floatValue() * length) / 100.0f) + floatValue;
        float f = 0.0f;
    ...
    public final void applyDashPatternIfNeeded(Matrix matrix) {
        ...
        for (int i = 0; i < this.dashPatternAnimations.size(); i++) {
            this.dashPatternValues[i] = this.dashPatternAnimations.get(i).mo190getValue().floatValue();
            if (i % 2 == 0) {
        ...
        BaseKeyframeAnimation<?, Float> baseKeyframeAnimation = this.dashPatternOffsetAnimation;
        this.paint.setPathEffect(new DashPathEffect(this.dashPatternValues, baseKeyframeAnimation == null ? 0.0f : scale * baseKeyframeAnimation.mo190getValue().floatValue()));
        L.endSection("StrokeContent#applyDashPattern");
    }

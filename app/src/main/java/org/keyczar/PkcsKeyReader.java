...
public class PkcsKeyReader implements KeyczarReader {
    ...
    private void ensureKeyRead() throws KeyczarException {
        try {
            ...
            KeyczarKey parseKeyStream = parseKeyStream(this.pkcs8Stream, this.passphrase, this.rsaPadding);
            this.key = parseKeyStream;
            this.meta = constructMetadata(parseKeyStream, this.purpose);
        } catch (IOException e) {
    ...
    private static byte[] decryptPbeEncryptedKey(byte[] bArr, String str) throws KeyczarException {
        ...
        try {
            ...
            cipher.init(2, computeDecryptionKey(str, algName), (PBEParameterSpec) encryptedPrivateKeyInfo.getAlgParameters().getParameterSpec(PBEParameterSpec.class));

...
public final class RealBufferedSource implements BufferedSource {
    ...
    public long read(Buffer buffer, long j) throws IOException {
        if (buffer != null) {
            if (j < 0) {
                ...
                if (buffer2.size == 0 && this.source.read(buffer2, FileAppender.DEFAULT_BUFFER_SIZE) == -1) {
                    return -1L;
                }
    ...
    public boolean exhausted() throws IOException {
        if (!this.closed) {
            return this.buffer.exhausted() && this.source.read(this.buffer, FileAppender.DEFAULT_BUFFER_SIZE) == -1;
        }
        throw new IllegalStateException("closed");
    }
    ...
    public boolean request(long j) throws IOException {
        ...
        if (j < 0) {
            ...
            } while (this.source.read(buffer, FileAppender.DEFAULT_BUFFER_SIZE) != -1);
            return false;
        }
    }
    ...
    public int select(Options options) throws IOException {
        ...
        } while (this.source.read(this.buffer, FileAppender.DEFAULT_BUFFER_SIZE) != -1);
        return -1;
    }
    ...
    public int read(byte[] bArr, int i, int i2) throws IOException {
        ...
        Buffer buffer = this.buffer;
        if (buffer.size == 0 && this.source.read(buffer, FileAppender.DEFAULT_BUFFER_SIZE) == -1) {
            return -1;
        }
        ...
        if (!this.closed) {
            ...
            while (j < j2) {
                ...
                if (indexOf == -1) {
                    Buffer buffer = this.buffer;
                    long j3 = buffer.size;
                    if (j3 >= j2 || this.source.read(buffer, FileAppender.DEFAULT_BUFFER_SIZE) == -1) {
                        break;
                    }
                    j = Math.max(j, j3);
                } else {
    ...
    public long indexOf(ByteString byteString, long j) throws IOException {
        if (!this.closed) {
            while (true) {
                ...
                Buffer buffer = this.buffer;
                long j2 = buffer.size;
                if (this.source.read(buffer, FileAppender.DEFAULT_BUFFER_SIZE) == -1) {
                    return -1L;
                }
